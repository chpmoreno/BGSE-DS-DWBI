...
select count(distinct(ProductID)) from products where UnitsOnOrder > 0;

-- 4) Show the number of products in stock (available right now) in our store

select count(distinct(ProductID)) from products where UnitsInStock > 0;

--  5) Show the number of products with more orders than stock

select count(distinct(ProductID)) from products where UnitsOnOrder > UnitsInStock;
